...
SELECT c.criminal_id, UPPER(c.last), UPPER(c.first), s.sentence_id, s.start_date, TO_CHAR(TO_DATE(start_date,'DD-MON-YY'), 'MONTH DD, YYYY') AS START_DATE,
ROUND(MONTHS_BETWEEN(s.end_date, s.start_date)) AS MONTHS_LENGTH FROM criminals c, sentences s WHERE c.criminal_id = s.criminal_id;

SELECT c.last ||' '|| c.first AS NAME, charge_id, TO_CHAR((fine_amount+court_fee), '$9999.99') AS "TOTAL_AMOUNT_OWED", 
PAY_DUE_DATE, TO_CHAR((fine_amount+court_fee)-AMOUNT_PAID, '$9999.99') AS AMOUNT_OWED,
CASE
WHEN AMOUNT_PAID = 0 THEN NULL
ELSE TO_CHAR(AMOUNT_PAID, '$9999.99')
END "AMOUNT_PAID"
FROM crime_charges r JOIN crimes i USING(crime_id) JOIN criminals c USING(criminal_id)
WHERE ((fine_amount+court_fee)-AMOUNT_PAID)>0;
 
SELECT c.last ||' '|| c.first AS NAME, s.start_date, MONTHS_BETWEEN(s.end_Date, s.start_date) "PROB_PERIOD",
ADD_MONTHS(s.start_date, -2) AS REVIEW_DATE FROM sentences s JOIN criminals c USING(criminal_id)
WHERE MONTHS_BETWEEN(s.end_Date, s.start_date) > 2 AND s.type = 'P';

SELECT status, COUNT(crime_id) FROM Crimes GROUP BY status;  

SELECT criminal_id, COUNT(crime_ID) FROM CRIMES GROUP BY CRIMINAL_ID;

SELECT MAX(COUNT(criminal_id)) AS NUM_CRIMES FROM crimes GROUP BY criminal_id;

SELECT MIN(fine_amount) AS LOWEST_FINE FROM crime_charges;

SELECT c.criminal_id, c.last ||' '|| c.first AS NAME, count(sentence_id) AS NUM_SENTENCES 
FROM criminals c JOIN senetences s USING(criminal_id) GROUP BY criminal_id HAVING COUNT(SENTENCE_ID); 
