...
BEGIN
    ...
    VALUES (BB_STATUS_SEQ.NEXTVAL,3,3,'20-FEB-12',NULL,'UPS','ZW2384YXK4957');
END;    

SELECT*FROM BB_BASKETSTATUS;

CREATE OR REPLACE PROCEDURE STATUS_SP
(IDB IN BB_BASKETSTATUS.IDBASKET%TYPE, DETAIL OUT VARCHAR2, ORDER_D OUT DATE)
IS
CURSOR CUR_STATUS IS
    SELECT IDSTAGE, MAX(DTSTAGE) FROM BB_BASKETSTATUS WHERE IDBASKET = IDB
    AND DTSTAGE = (SELECT MAX(DISTINCT DTSTAGE) FROM BB_BASKETSTATUS WHERE IDBASKET = IDB)
    GROUP BY IDSTAGE;
    IDS BB_BASKETSTATUS.IDSTAGE%TYPE;
BEGIN
    OPEN CUR_STATUS; -- THANH NGUYEN
    FETCH CUR_STATUS INTO IDS, ORDER_D; 
    CASE
        WHEN IDS = 1 THEN DETAIL :='Submitted and received';
        WHEN IDS = 2 THEN DETAIL :='Confirmed, processed, sent to shipping';
        WHEN IDS = 3 THEN DETAIL :='Shipped';
        WHEN IDS = 4 THEN DETAIL :='Cancelled';
        WHEN IDS = 5 THEN DETAIL :='Back-ordered';
    END CASE;
    EXCEPTION 
        WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('No status is available');
    DBMS_OUTPUT.PUT_LINE('Recent order is '||DETAIL||' on '|| ORDER_D);    
    CLOSE CUR_STATUS; 
END;
...
IS
CURSOR CUR_PROMO IS
    SELECT DISTINCT(IDSHOPPER), MAX(DTCREATED) FROM BB_BASKET GROUP BY IDSHOPPER HAVING MAX(DTCREATED) < SPEC_DATE;
    TYPE TYPE_PROMO IS RECORD (IDS BB_BASKET.IDSHOPPER%TYPE, LAST BB_BASKET.DTCREATED%TYPE);
    REC_PROMO TYPE_PROMO;
    
    MON CHAR(3) := 'APR'; YR CHAR(4) := '2012'; FLAG CHAR(2) := '1'; USED CHAR(1) := 'N';
BEGIN
    OPEN CUR_PROMO; -- THANH NGUYEN
        LOOP
        FETCH CUR_PROMO INTO REC_PROMO;
            INSERT INTO BB_PROMOLIST VALUES (REC_PROMO.IDS, MON, YR, FLAG, USED);
...
DECLARE 
    ...
    MON CHAR(3) := 'APR'; YR CHAR(4) := '2012'; FLAG CHAR(2) := '1'; USED CHAR(1) := 'N'; 
BEGIN
    PROMO_SHIP_SP(SPEC_DATE);
    INSERT INTO BB_PROMOLIST VALUES (IDSHOPPER, MON, YR, FLAG, USED);
END;


CREATE OR REPLACE PROCEDURE BASKET_ADD_SP
    (IDB IN NUMBER, IDP IN NUMBER, PRICE IN NUMBER, QTY IN NUMBER, SIZE_CODE IN NUMBER, FORM_CODE IN NUMBER)
IS
BEGIN
    INSERT INTO BB_BASKETITEM(IDBASKETITEM, IDPRODUCT, PRICE, QUANTITY, OPTION1, OPTION2)
    VALUES (BB_IDBASKETITEM_SEQ.NEXTVAL, 8, 10.80, 1,2,4);
END;    


CREATE OR REPLACE PROCEDURE MEMBER_CK_SP
    (USER_ID IN VARCHAR2, USER_PW IN VARCHAR2, USER_COOKIE OUT NUMBER, P_CHECK OUT VARCHAR2)
IS
CURSOR CUR_USER IS
    SELECT FIRSTNAME, LASTNAME, COOKIE, USERNAME, PASSWORD FROM BB_SHOPPER WHERE USERNAME = USER_ID AND PASSWORD=USER_PW; 
BEGIN
    FOR REC_USER IN CUR_USER LOOP
        IF USER_ID=REC_USER.USERNAME AND USER_PW=REC_USER.PASSWORD THEN
            P_CHECK := 'VALID';
            DBMS_OUTPUT.PUT_LINE(REC_USER.FIRSTNAME||' '||REC_USER.LASTNAME||' '||REC_USER.COOKIE||' '||P_CHECK);  
        ELSE
            P_CHECK := 'INVALID';
...
DECLARE 
    COOK bb_shopper.cookie%TYPE; 
    TEXT VARCHAR2(7); 
BEGIN 
    MEMBER_CK_SP('rat55','kile',COOK,TEXT); 
END; 
...
DECLARE 
    ...
    TEXT VARCHAR2(10); 
BEGIN 
    MEMBER_CK_SP('rat','kie',COOK,TEXT); 
END; 
