...
CREATE OR REPLACE PROCEDURE DDPROJ_SP 
    (IDP IN DD_PROJECT.IDPROJ%TYPE, DATAP OUT DD_PROJECT%ROWTYPE)
IS
BEGIN
    SELECT*INTO DATAP FROM DD_PROJECT WHERE IDPROJ = IDP;
    DBMS_OUTPUT.PUT_LINE(DATAP.IDPROJ||' '||DATAP.PROJNAME||' '||DATAP.PROJSTARTDATE||' '||
    DATAP.PROJENDDATE||' '||DATAP.PROJFUNDGOAL||' '||DATAP.PROJCOORD);
END;

DECLARE
    TYPE LIST_P IS RECORD(
...
CREATE OR REPLACE PROCEDURE DDPAY_SP
    (IDD IN NUMBER, ACTIVE OUT BOOLEAN)
IS
CURSOR CUR_PLED IS
    SELECT IDSTATUS, PAYMONTHS FROM DD_PLEDGE WHERE IDDONOR = IDD;
    STATUS NUMBER;
    PAY NUMBER;
BEGIN
    OPEN CUR_PLED;
    FETCH CUR_PLED INTO STATUS, PAY;
        IF PAY = 0 OR STATUS = 20 THEN
            ACTIVE := FALSE;
        ELSE ACTIVE := TRUE;    
        END IF; 
    CLOSE CUR_PLED;
